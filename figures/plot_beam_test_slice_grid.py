...
BEAM_DIR = (
    ...
    / "beam"
    / "outputs"
    / "offline_encoder_rollout_duffing_video_sindy_2026_06_12_10_28_39_best"
)

PNG_PATH = OUTPUT_DIR / "beam_test_slice_grid_true_pred_10x10.png"
PDF_PATH = OUTPUT_DIR / "beam_test_slice_grid_true_pred_10x10.pdf"
...
def prediction_files() -> list[Path]:
    files = sorted(BEAM_DIR.glob("offline_prediction_beam_*.npz"))[:10]
    if len(files) != 10:
        raise FileNotFoundError(f"Expected 10 beam prediction files in {BEAM_DIR}, got {len(files)}")
    return files
...
def build_figure(save: bool = True, show: bool = False) -> plt.Figure:
    ...
    fig.text(
        0.5,
        0.985,
        "Forced cantilever beam test-set slices: true / pred",
        ha="center",
        va="top",
        fontsize=11.4,
        color=TEXT_COLOR,
